...
public class Utils {
	public static void connectNodes(int start, int end){	
		Node localNode1 = Simulator.nodeList.get(start);
		Node localNode2 = Simulator.nodeList.get(end);
	...
	public static void connectNodes(Node start, Node end){
		...
		Simulator.edgeList.put(new Edge(start, end), Boolean.TRUE);
		Simulator.edgeList.put(new Edge(end, start), Boolean.TRUE);
	}
	...
	public static void moveNodes(Vector<Node> nodeList,Vector3d size){
			...
			while (localEnumeration.hasMoreElements()){
				...
				if ((node.getInfo().getPosition().x < 0.0D) || (node.getInfo().getPosition().x > size.x)){
					node.getInfo().xMove *= -1.0D;
					node.getInfo().getPosition().x += node.getInfo().xMove;
					node.getInfo().getPosition().x += node.getInfo().xMove;
				}
				if ((node.getInfo().getPosition().y >= 0.0D) && (node.getInfo().getPosition().y <= size.y)){
					continue;
				}
