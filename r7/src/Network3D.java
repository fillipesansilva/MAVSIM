...
public class Network3D extends JComponent {
	...
	protected Color3f agentColor3;
	protected Color3f agentColor4;
	protected Color3f agentColor5;
	...
	Appearance agentAppear4 = new Appearance();
	Appearance agentAppear5 = new Appearance();
	...
	public void updateAgentList(Vector<Agent> agentVector){
		Simulator.get().agentList = agentVector;
	}
	...
	public Network3D(Vector<Node> nodesVector, Vector<Agent> agentVector, int sizeX, int sizeY){
		...
		this.nodeColor2 = new Color3f(1F, 0.5F, 0);// Orange
		this.nodeColor3 = new Color3f(1F, 0, 0); // RED
		this.linkColor = new Color3f(1F, 1F, 1F); // WHITE;
		this.txRangeColor = new Color3f(0.2F, 0.2F, 0.2F);// GREY

		this.agentColor = new Color3f(0, 1F, 0);// GREEN
		this.agentColor2 = new Color3f(1F, 1F, 0);// YELLOW
		this.agentColor3 = new Color3f(0, 0, 1F); // BLUE
		this.agentColor4 = new Color3f(1F, 0, 1F);// GREEN
		this.agentColor5 = new Color3f(0, 1F, 1F);// YELLOW


		this.agentDead = new Color3f(1F, 0, 0);// GREEN
		...
		agentAppear4.setCapability(Appearance.ALLOW_COLORING_ATTRIBUTES_WRITE);
		agentAppear5.setCapability(Appearance.ALLOW_COLORING_ATTRIBUTES_WRITE);
		agentAppearBH.setCapability(Appearance.ALLOW_COLORING_ATTRIBUTES_WRITE);
		agentAppear1.setColoringAttributes(new ColoringAttributes(agentColor, ColoringAttributes.FASTEST));
		agentAppear2.setColoringAttributes(new ColoringAttributes(agentColor2, ColoringAttributes.FASTEST));
		agentAppear3.setColoringAttributes(new ColoringAttributes(agentColor3, ColoringAttributes.FASTEST));
		agentAppear4.setColoringAttributes(new ColoringAttributes(agentColor4, ColoringAttributes.FASTEST));
		agentAppear5.setColoringAttributes(new ColoringAttributes(agentColor5, ColoringAttributes.FASTEST));
	...
	public void renderNodes(Graphics paramGraphics) {
		if(Simulator.get().agentList.size()>0){//if there are still agents
			for (Node node : Simulator.get().nodeList) {
				drawNode(node);
			}
		}
	}
	...
	public void renderState(Graphics paramGraphics) {
		...
		Enumeration<Node> localEnumeration = Simulator.get().nodeList.elements();
		...
		if(Simulator.get().agentList.size()>0){//if there are still agents
			...
			while (localEnumeration.hasMoreElements()) {
				...
				float f = (Simulator.get().agentList.size() - localNode.getInfo().knownBy) / Simulator.get().agentList.size();

				if(localNode.getBH()){
					localNode.changeColorNode(new Color3f(
							this.nodeColor3.x * 1f,
							this.nodeColor3.y * 0f,
							this.nodeColor3.z * 0f));
					...
					localNode.changeColorNode(new Color3f(
							this.nodeColor1.x * 1f,
							this.nodeColor1.y * 1f,
							this.nodeColor1.z * 1f));
				}
			}//while

		}

		Enumeration<Agent> agentes = Simulator.get().agentList.elements();
		...
		while (agentes.hasMoreElements()) {
			...
			if(a.exit){
				//int idAgent=a.getBehaviour().getParent()
				publicationNode.removeChild(a.getBehaviour().getParent());
	...
	public void renderRangeAndLinks(Graphics paramGraphics) {
		...
		Enumeration<Node> localEnumeration1 = Simulator.get().nodeList.elements();
		...
		localEnumeration1 = Simulator.get().nodeList.elements();
		while (localEnumeration1.hasMoreElements()) {
			localNode1 = (Node) localEnumeration1.nextElement();
			// localNodeInfo1 = (NodeInfo)this.nodeInfoTable.get(localNode1);
		...
		Enumeration<Agent> agents = Simulator.get().agentList.elements();

		while (agents.hasMoreElements()) {
			drawAgent(agents.nextElement());
		}
	...
	protected void drawAgent(Agent agent) {
		...
		if(agent.executing){
			...
			if(color==0){
				...
				sphere = new Sphere(0.04f, agentAppear4);
			}else if(color==4){
				sphere = new Sphere(0.04f, agentAppear5);
			}
		}else{
			sphere = new Sphere(0.02f, agentAppearBH);
		}

		BranchGroup branchAgent=new BranchGroup();
		branchAgent.setCapability(BranchGroup.ALLOW_DETACH);
		branchAgent.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
		branchAgent.setCapability(BranchGroup.ALLOW_PICKABLE_WRITE);
		...
		moveAgent.setCapability(TransformGroup.ALLOW_CHILDREN_WRITE);
		moveAgent.setCapability(TransformGroup.ALLOW_PICKABLE_WRITE);
		...
		branchAgent.addChild(moveAgent);
		publicationNode.addChild(branchAgent);
